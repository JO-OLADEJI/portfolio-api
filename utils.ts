...
import { MeetingContact, FormContact, TerminalContact } from "./types";
...
export const dispatchMailToCDGR = async (
  details: MeetingContact | FormContact | TerminalContact
): Promise<boolean> => {
  const source =
    details.src === "meeting"
      ? "meeting"
      : details.src === "terminal"
      ? "terminal"
      : "form";
  const fmtSource =
  ...
  const mailOptions: SendMailOptions = {
    from: `"TheCodeographer 🙋🏽‍♂️" <${process.env.CDGR_RESPONDER}>`,
